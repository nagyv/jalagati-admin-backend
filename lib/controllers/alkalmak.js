...
function addResztvevo(req, reply) {
  ...
  req.pre.alkalom.addResztvevo(jogas, function(err, resztvevo) {
    if (err) {
      reply({error: err.errors});
    } else {
      resztvevo.populate('jogas', function(err, resztvevo){
        reply(resztvevo);
      });
    }
  });
...
function removeResztvevo(req, reply) {
  var jogas = req.pre.jogas;
  console.log(jogas);
  req.pre.alkalom.removeResztvevo(jogas, function(err){
    if (err) {
      reply({error: err.errors});
    } else {
      reply();
